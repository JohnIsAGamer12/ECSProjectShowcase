...
#pragma endregion
	...
	void Update_VulkanRenderer(entt::registry& registry, entt::entity entity)
	{
		...
		// Group all Entites in the World in a container to sort out the geometry data
		...
		std::vector<GPUInstance> instanceBuffers;
		std::map<int, GeometryData> geometryMap;

		int gpuIndex = 0;

		for (auto& instance : instances)
		{
			instanceBuffers.push_back(instances.get<GPUInstance>(instance));
			geometryMap[gpuIndex] = instances.get<GeometryData>(instance);

			++gpuIndex;
		}

		auto& storageBuffer = registry.emplace<std::vector<GPUInstance>>(entity, instanceBuffers);

		registry.patch<VulkanGPUInstanceBuffer>(entity, [&](auto& storageBuffer) { });
		...
		if (registry.all_of< VulkanVertexBuffer, VulkanIndexBuffer>(entity))
		{
			...
			for (int i = 0; i < geometryMap.size(); i++)
			{
				GeometryData temp = geometryMap[i];
				vkCmdDrawIndexed(commandBuffer, temp.indexCount, 1, temp.indexStart, temp.vertexStart, i);
			}
